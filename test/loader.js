(function() {
  ...
  window.dc.embed.loadNote = function(url, options) {
    if (window.dc.noteEmbedJSParsed) {
      ...
      var noteLoadingUtils = {

        samizdatOn: function (el, eventName, handler) {
        ...
        insertJavaScript: function(src, onLoadCallback) {
          if (!document.querySelector('script[src$="' + src + '"]')) {
            script = document.createElement('script');
            script.src = src;
            script.async = true;
            this.samizdatOn(script, 'load', onLoadCallback);
      ...
      // 3. Insert the styles and scripts needed, with `loadNotes` fired once 
      // the script has finished loading
      noteLoadingUtils.insertStylesheet('../dist/note_embed.css');
      noteLoadingUtils.insertJavaScript('../dist/note_embed.js', noteLoadingUtils.loadNotes);
    }
  }
